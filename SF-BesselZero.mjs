...
import { gsl_sf_bessel_Olver_zofmzeta }    from "./SF-BesselOlver.mjs";
...
//TYPE PSeries IS ACCESS CONSTANT Series;
...
function clenshaw( c, N, u )
{
    ...
    for ( let i = N; i >= 1; i-- )
    {
        B_nm1 = 2.0 * (2.0 * u - 1.0) * B_n - B_np1 + c[(i-1)];
        B_np1 = B_n;
        B_n   = B_nm1;
    }
...
//
//static double
function mcmahon_correction( mu, beta )
{
    const eb   = 8.0 * beta;
    const ebsq = eb * eb;
    ...
    if ( mu < GSL_DBL_EPSILON )
    {
        ...
        const term1 =  1.0 / ebsq;
        const term2 = -4.0 * 31.0 / (3.0 * ebsq * ebsq);
        const term3 =  32.0 * 3779.0 / (15.0 * ebsq * ebsq * ebsq);
        const term4 = -64.0 * 6277237.0 / (105.0 * ebsq * ebsq * ebsq * ebsq);
        const term5 =  512.0 * 2092163573.0 / (315.0 * ebsq * ebsq * ebsq * ebsq * ebsq);
    ...
    else
    {
        ...
        const mi    = 1.0 / mu;
        const r     = mu / ebsq;
        const n2    = 4.0 / 3.0    * (7.0 - 31.0*mi);
        const n3    = 32.0 / 15.0  * (83.0 + (-982.0 + 3779.0 * mi) * mi);
        const n4    = 64.0 / 105.0 * (6949.0 + (-153855.0 + (1585743.0 - 6277237.0 * mi) * mi) * mi);
        const n5    = 512.0 / 315.0 * (70197.0 + (-2479316.0 + (48010494.0 + (-512062548.0 + 2092163573.0 * mi) * mi) * mi) * mi);
        const n6    = 2048.0 / 3465.0 * (5592657.0 + (-287149133.0 + (8903961290.0 + (-179289628602.0 + (1982611456181.0 - 8249725736393.0 * mi) * mi) * mi) * mi) * mi);
        const term1 = (1.0 - mi) * r;
        const term2 = term1 * n2 * r;
        const term3 = term1 * n3 * r * r;
        const term4 = term1 * n4 * r * r * r;
        const term5 = term1 * n5 * r * r * r * r;
        const term6 = term1 * n6 * r * r * r * r * r;
...
///* Assumes z >= 1.0 */
function olver_b0( z, minus_zeta )
{
    ...
    {
        var abs_zeta = minus_zeta;
        var t = 1.0 / (z * Math.sqrt( 1.0 - 1.0 / (z * z) ));
...
function olver_f1( z, minus_zeta )
{
    var b0 = olver_b0( z, minus_zeta );
    var h2 = Math.sqrt( 4.0 * minus_zeta / (z * z - 1.0) ); // FIXME
...
export function gsl_sf_bessel_zero_J0_e( s )
{
    ...
    else
    {
        ...
        var beta   = ((s) - 0.25) * M_PI;
        var bi2    = 1.0 / (beta * beta);
        var R33num = P[0] + bi2 * (P[1] + bi2 * (P[2] + P[3] * bi2));
        var R33den = Q[0] + bi2 * (Q[1] + bi2 * (Q[2] + Q[3] * bi2));
        var R33    = R33num / R33den;

        r.val = beta + R33 / beta;
        r.err = Math.abs( 3.0e-15 * r.val );
    }
...
export function gsl_sf_bessel_zero_J1_e( s )
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    else
    {
        ...
        var beta = ((s) + 0.25) * M_PI;
        var bi2  = 1.0 / (beta * beta);
        var Rnum = a[3] + bi2 * (a[2] + bi2 * (a[1] + bi2 * a[0]));
        var Rden = b[3] + bi2 * (b[2] + bi2 * (b[1] + bi2 * b[0]));
        var S    = Rnum / Rden;

        r.val = beta * (1.0 + S * bi2);
        r.err = Math.abs( 2.0e-14 * r.val );
    }
...
export function gsl_sf_bessel_zero_Jnu_e( nu, s )
{
    var c   = null;
    var L   = 0;
    var arg = 0.0;
    ...
    else if ( s == 0 )
    {
        ...
        if ( nu == 0.0 )
        {
            ...
            throw "SF.GenericException"; // WITH "no zero-th root for nu = 0.0";
        }
    }
    else if ( nu < 0.0 )
    {
    ...
    else if ( (s) > 2.0 * nu )
    {
        ...
        var beta  = 0.0;
        var mc    = 0.0;
        var rat12 = { val: 0.0, err: 0.0 }; // Result;

        beta = ((s) + 0.5 * nu - 0.25) * M_PI;
        mc   = mcmahon_correction( 4.0 * nu * nu, beta );
        rat12 = gsl_sf_pow_int_e( nu / beta, 14 );
        r.val = beta * mc;
        r.err = 4.0 * Math.abs( beta ) * rat12.val;
        r.err = r.err + 4.0 * Math.abs( GSL_DBL_EPSILON * r.val );
    }
    else
    {
        // Olver uniform asymptotic.
        var as = { val: 0.0, err: 0.0 }; // Result;
        var minus_zeta = 0.0;
        var z  = 0.0;
        var f1 = 0.0;

        as = gsl_sf_airy_zero_Ai_e( s );
        minus_zeta = -Math.pow(nu, (-2.0 / 3.0)) * as.val;
        z = gsl_sf_bessel_Olver_zofmzeta( minus_zeta );
        f1 = olver_f1( z, minus_zeta );
        r.val = nu * (z + f1 / (nu * nu));
        r.err = 0.001 / (nu * nu * nu);
...
export function gsl_sf_bessel_zero_J0( s )
{ // gsl_sf_bessel_zero_J0
    return EVAL_RESULT_I( gsl_sf_bessel_zero_J0_e, s, "gsl_sf_bessel_zero_J0" );
} // gsl_sf_bessel_zero_J0;

export function gsl_sf_bessel_zero_J1( s )
{ // gsl_sf_bessel_zero_J1
    return EVAL_RESULT_I( gsl_sf_bessel_zero_J1_e, s, "gsl_sf_bessel_zero_J1" );
} // gsl_sf_bessel_zero_J1;

export function gsl_sf_bessel_zero_Jnu( nu, s )
{ // gsl_sf_bessel_zero_Jnu
    return EVAL_RESULT_DI( gsl_sf_bessel_zero_Jnu_e, { x: nu, i: s }, "gsl_sf_bessel_zero_Jnu" );
} // gsl_sf_bessel_zero_Jnu;

// ----------------------------------------------------------------------------
// EOF SF-BesselZero.mjs
