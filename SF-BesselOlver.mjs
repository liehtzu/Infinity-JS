...
const zofmzeta_a_cs = { length: 19, c: zofmzeta_a_data, order: 19, a: -1.0, b: 1.0, order_sp: 8 };


// fit for f(x) = zofmzeta((9x+11)/2), 1 <= mzeta <= 10
const zofmzeta_b_data =
    [
...
const zofmzeta_b_cs = { length: 29, c: zofmzeta_b_data, order: 29, a: -1.0, b: 1.0, order_sp: 15 };
...
const zofmzeta_c_cs = { length: 10, c: zofmzeta_c_data, order: 10, a: -1.0, b: 1.0, order_sp: 6 };


// Invert [Abramowitz+Stegun, 9.3.39].
// Assumes minus_zeta >= 0.
//
export function gsl_sf_bessel_Olver_zofmzeta( minus_zeta )
{
    const TEN_32 = 31.62277660168379332; // 10^(3/2)
    ...
    if ( minus_zeta < 1.0 )
    {
        ...
        c = cheb_eval_e( zofmzeta_a_cs, x );
        return c.val;
    }
    else if ( minus_zeta < 10.0 )
    {
        x = (2.0 * minus_zeta - 11.0) / 9.0;
        c = cheb_eval_e( zofmzeta_b_cs, x );
        return c.val;
    }
    else
    {
...
const A3_gt1_cs = { length: 30, c: A3_gt1_data, order: 30, a: -1.0, b: 1.0, order_sp: 17 };
...
const A4_gt1_cs = { length: 29, c: A4_gt1_data, order: 17, a: -1.0, b: 1.0, order_sp: 17 }; // 17 --> 29
...
const B2_gt1_cs = { length: 39, c: B2_gt1_data, order: 39, a: -1.0, b: 1.0, order_sp: 30 };
...
const B3_gt1_cs = { length: 29, c: B3_gt1_data, order: 29, a: -1.0, b: 1.0, order_sp: 29 };
...
const B2_lt1_cs = { length: 39, c: B2_lt1_data, order: 39, a: -1.0, b: 1.0, order_sp: 39 };
...
const B3_lt1_cs = { length: 39, c: B3_lt1_data, order: 39, a: -1.0, b: 1.0, order_sp: 39 };
...
const A3_lt1_cs = { length: 39, c: A3_lt1_data, order: 39, a: -1.0, b: 1.0, order_sp: 39 };
...
const A4_lt1_cs = { length: 29, c: A4_lt1_data, order: 29, a: -1.0, b: 1.0, order_sp: 29 };

// ----------------------------------------------------------------------------

function olver_B0(z, abs_zeta)
{

    if (z < 0.98)
    {
        let t = 1.0 / Math.sqrt(1.0 - z * z);
        return -5.0 / (48.0 * abs_zeta * abs_zeta) + t * (-3.0 + 5.0 * t * t) / (24.0 * Math.sqrt(abs_zeta));
    }
    else if (z < 1.02)
    {
        const a  = 1.0 - z;
        const c0 =  0.0179988721413553309252458658183;
        const c1 =  0.0111992982212877614645974276203;
        const c2 =  0.0059404069786014304317781160605;
        const c3 =  0.0028676724516390040844556450173;
        const c4 =  0.0012339189052567271708525111185;
        const c5 =  0.0004169250674535178764734660248;
        const c6 =  0.0000330173385085949806952777365;
        const c7 = -0.0001318076238578203009990106425;
        const c8 = -0.0001906870370050847239813945647;

        return c0 + a * (c1 + a * (c2 + a * (c3 + a * (c4 + a * (c5 + a * (c6 + a * (c7 + a * c8)))))));
    }
    else
    {
        let t = 1.0 / (z * Math.sqrt(1.0 - 1.0 / (z * z)));
        return -5.0 / (48.0 * abs_zeta * abs_zeta) + t * (3.0 + 5.0 * t * t) / (24.0 * Math.sqrt(abs_zeta));
    }
...
function olver_B1(z, abs_zeta)
{

    if (z < 0.88)
    {
        const t   = 1.0 / Math.sqrt(1.0 - z * z);
        const t2  = t * t;
        const rz  = Math.sqrt(abs_zeta);
        const z32 = rz * rz * rz;
        const z92 = z32 * z32 * z32;
        const term1 = t * t * t * (30375.0 - 369603.0 * t2 + 765765.0 * t2 * t2 - 425425.0 * t2 * t2 * t2) / 414720.0;
        const term2 = 85085.0 / (663552.0 * z92);
        const term3 = 385.0 / 110592.0 * t * (3.0 - 5.0 * t2) / (abs_zeta * abs_zeta * abs_zeta);
        const term4 = 5.0 / 55296.0 * t2 * (81.0 - 462.0 * t2 + 385.0 * t2 * t2) / z32;
    ...
    {
        const a   = 1.0 - z;
        const c0  = -0.00149282953213429172050073403334;
        const c1  = -0.00175640941909277865678308358128;
        const c2  = -0.00113346148874174912576929663517;
        const c3  = -0.00034691090981382974689396961817;
        const c4  =  0.00022752516104839243675693256916;
        const c5  =  0.00051764145724244846447294636552;
        const c6  =  0.00058906174858194233998714243010;
        const c7  =  0.00053485514521888073087240392846;
        const c8  =  0.00042891792986220150647633418796;
        const c9  =  0.00031639765900613633260381972850;
        const c10 =  0.00021908147678699592975840749194;

        return c0 + a * (c1 + a * (c2 + a * (c3 + a * (c4 + a * (c5 + a * (c6 + a * (c7 + a * (c8 + a * (c9 + a * c10)))))))));
    }
    else
    {
        const t   = 1.0 / (z * Math.sqrt(1.0 - 1.0 / (z * z)));
        const t2  = t * t;
        const rz  = Math.sqrt(abs_zeta);
        const z32 = rz * rz * rz;
        const z92 = z32 * z32 * z32;
        const term1 = -t2 * t * (30375.0 + 369603.0 * t2 + 765765.0 * t2 * t2 + 425425.0 * t2 * t2 * t2) / 414720.0;
        const term2 = 85085.0 / (663552.0 * z92);
        const term3 = -385.0 / 110592.0 * t * (3.0 + 5.0 * t2) / (abs_zeta * abs_zeta * abs_zeta);
        const term4 = 5.0 / 55296.0 * t2 * (81.0 + 462.0 * t2 + 385.0 * t2 * t2) / z32;
...
function olver_B2(z, abs_zeta)
{

    if (z < 0.8)
    {
        const x = 5.0 * z / 2.0 - 1.0;
        let c = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(B2_lt1_cs, x);
        return  c.val / z;
    }
    else if (z <= 1.2)
    {
        const a  = 1.0 - z;
        const c0 = 0.00055221307672129279005986982501;
        const c1 = 0.00089586516310476929281129228969;
        const c2 = 0.00067015003441569770883539158863;
        const c3 = 0.00010166263361949045682945811828;
        const c4 = -0.00044086345133806887291336488582;
        const c5 = -0.00073963081508788743392883072523;
        const c6 = -0.00076745494377839561259903887331;
        const c7 = -0.00060829038106040362291568012663;
        const c8 = -0.00037128707528893496121336168683;
        const c9 = -0.00014116325105702609866850307176;
    ...
    {
        const zi = 1.0 / z;
        const x  = 12.0 / 5.0 * zi - 1.0;
        let c = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(B2_gt1_cs, x);
...
function olver_B3(z, abs_zeta)
{
    ...
    if (z < 0.8)
    {
        ...
        c = cheb_eval_e(B3_lt1_cs, x);
        return c.val;
    }
    else if (z < 1.2)
    {
        const a  = 1.0 - z;
        const c0 = -0.00047461779655995980754441833105;
        const c1 = -0.00095572913429464297452176811898;
        const c2 = -0.00080369634512082892655558133973;
        const c3 = -0.00000727921669154784138080600339;
        const c4 =  0.00093162500331581345235746518994;
        const c5 =  0.00149848796913751497227188612403;
        const c6 =  0.00148406039675949727870390426462;

        return c0 + a * (c1 + a * (c2 + a * (c3 + a * (c4 + a * (c5 + a * c6)))));
    }
    else
    {
        const x   = 12.0 / (5.0 * z) - 1.0;
        const zi2 = 1.0 / (z * z);
        let c = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(B3_gt1_cs, x);
...
function olver_A1(z, abs_zeta) //: LONG_FLOAT; val: IN OUT LONG_FLOAT; err: IN OUT LONG_FLOAT) IS
{
    ...
    if (z < 0.98)
    {
        ...
        const rz = Math.sqrt(abs_zeta);
        const t2 = t * t;
        const term1 =  t2 * (81.0 - 462.0 * t2 + 385.0 * t2 * t2) / 1152.0;
        const term2 = -455.0 / (4608.0 * abs_zeta * abs_zeta * abs_zeta);
        const term3 =  7.0 * t * (-3.0 + 5.0 * t2) / (1152.0 * rz * rz * rz);

        r.err = 2.0 * GSL_DBL_EPSILON * (Math.abs(term1) + Math.abs(term2) + Math.abs(term3));
        r.val = term1 + term2 + term3;
    }
    else if (z < 1.02)
    {
        const a  = 1.0 - z;
        const c0 = -0.00444444444444444444444444444444;
        const c1 = -0.00184415584415584415584415584416;
        const c2 =  0.00056812076812076812076812076812;
        const c3 =  0.00168137865661675185484709294233;
        const c4 =  0.00186744042139000122193399504324;
        const c5 =  0.00161330105833747826430066790326;
        const c6 =  0.00123177312220625816558607537838;
        const c7 =  0.00087334711007377573881689318421;
        const c8 =  0.00059004942455353250141217015410;
        const sum = c0 + a * (c1 + a * (c2 + a * (c3 + a * (c4 + a * (c5 + a * (c6 + a * (c7 + a * c8)))))));
...
function olver_A2(z, abs_zeta)
{
    ...
    if (z < 0.88)
    {
        ...
        const t8 = t4 * t4;
        const rz = Math.sqrt(abs_zeta);
        const z3 = abs_zeta * abs_zeta * abs_zeta;
        const z32 = rz * rz * rz;
        const z92 = z3 * z32;
        const term1 = t4 * (4465125.0 - 94121676.0 * t2 + 349922430.0 * t4 - 446185740.0 * t6  + 185910725.0 * t8) / 39813120.0;
        const term2 = -40415375.0 / (127401984.0 * z3 * z3);
        const term3 = -95095.0 / 15925248.0 * t * (3.0 - 5.0 * t2) / z92;
        const term4 = -455.0 / 5308416.0 * t2 * (81.0 - 462.0 * t2 + 385.0 * t4) / z3;
        const term5 = -7.0 / 19906560.0 * t * t2 * (30375.0 - 369603.0 * t2  + 765765.0 * t4  - 425425.0 * t6) / z32;
    ...
    {
        const a   = 1.0 - z;
        const c0  =  0.000693735541354588973636592684210;
        const c1  =  0.000464483490365843307019777608010;
        const c2  = -0.000289036254605598132482570468291;
        const c3  = -0.000874764943953712638574497548110;
        const c4  = -0.001029716376139865629968584679350;
        const c5  = -0.000836857329713810600584714031650;
        const c6  = -0.000488910893527218954998270124540;
        const c7  = -0.000144236747940817220502256810151;
        const c8  =  0.000114363800986163478038576460325;
        const c9  =  0.000266806881492777536223944807117;
        const c10 = -0.011975517576151069627471048587000;
...
function olver_A3(z, abs_zeta)
{

    if (z < 0.9)
    {
        const x = 20.0 * z / 9.0 - 1.0;
        let c = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(A3_lt1_cs, x);
        return c.val;
    }
    else if (z < 1.1)
    {
        const a  = 1.0 - z;
        const c0 = -0.000354211971457743840771125759200;
        const c1 = -0.000312322527890318832782774881353;
        const c2 =  0.000277947465383133980329617631915;
        const c3 =  0.000919803044747966977054155192400;
        const c4 =  0.001147600388275977640983696906320;
        const c5 =  0.000869239326123625742931772044544;
        const c6 =  0.000287392257282507334785281718027;

        return c0 + a * (c1 + a * (c2 + a * (c3 + a * (c4 + a * (c5 + a * c6)))));
    }
    else
    {
        const x   = 11.0 / (5.0 * z) - 1.0;
        const zi2 = 1.0 / (z * z);
        let c   = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(A3_gt1_cs, x);
        return  c.val * zi2 * zi2 * zi2;
    }
...
function olver_A4(z, abs_zeta)
{
    ...
    if (z < 0.8)
    {
        ...
        c = cheb_eval_e(A4_lt1_cs, x);
        return c.val;
    }
    else if (z < 1.2)
    {
        const a  = 1.0 - z;
        const c0 =  0.00037819419920177291402661228437;
        const c1 =  0.00040494390552363233477213857527;
        const c2 = -0.00045764735528936113047289344569;
        const c3 = -0.00165361044229650225813161341879;
        const c4 = -0.00217527517983360049717137015539;
        const c5 = -0.00152003287866490735107772795537;
...
function olver_Asum(nu, z, abs_zeta) //: LONG_FLOAT; ret: IN OUT LONG_FLOAT; err: IN OUT LONG_FLOAT) IS
{

    var nu2 = 0.0;
    var A1  = { val: 0.0, err: 0.0 };
    var A2  = 0.0;
    var A3  = 0.0;
    var A4  = 0.0;
    var r = { val: 0.0, err: 0.0 };
    ...
    nu2 = nu * nu;
    A1 = olver_A1(z, abs_zeta); //, A1, A1_err);
    A2  = olver_A2(z, abs_zeta);
    A3  = olver_A3(z, abs_zeta);
    A4  = olver_A4(z, abs_zeta);
    r.err = A1.err / nu2 + GSL_DBL_EPSILON;
    r.val = 1.0 + A1.val / nu2 + A2 / (nu2 * nu2) + A3 / (nu2 * nu2 * nu2) + A4/(nu2 * nu2 * nu2 * nu2);
    return r;
...
function olver_Bsum(nu, z, abs_zeta)
{
    var nu2 = 0.0;
    var B0  = 0.0;
    var B1  = 0.0;
    ...
    B2  = olver_B2(z, abs_zeta);
    B3  = olver_B3(z, abs_zeta);
...
export function gsl_sf_bessel_Jnu_asymp_Olver_e(nu, x)
{
    ...
    else
    {
        ...
        var aip = { val: 0.0, err: 0.0 }; // Result;
        var asm = { val: 0.0, err: 0.0 }; // Result;
        ...
        {
            const a  = 1.0 - z;
            const c0 = 1.25992104989487316476721060728;
            const c1 = 0.37797631496846194943016318218;
            const c2 = 0.230385563409348235843147082474;
            const c3 = 0.165909603649648694839821892031;
            const c4 = 0.12931387086451008907;
            const c5 = 0.10568046188858133991;
            const c6 = 0.08916997952268186978;
            const c7 = 0.07700014900618802456;
        ...
        asm = olver_Asum(nu, z, abs_zeta); //, asum, asum_err);
        asum = asm.val;
        asum_err = asm.err;
        bsum = olver_Bsum(nu, z, abs_zeta);
        
        arg  = crnu * crnu * zeta;
        ai  = gsl_sf_airy_Ai_e(arg, GSL_MODE_DEFAULT);
        aip = gsl_sf_airy_Ai_deriv_e(arg, GSL_MODE_DEFAULT);
        
        r.val = pre * (ai.val * asum / crnu + aip.val * bsum / (nu * crnu * crnu));
        r.err = pre * (ai.err * Math.abs(asum / crnu));
        r.err = r.err + pre * Math.abs(ai.val) * asum_err / crnu;
        r.err = r.err + pre * Math.abs(ai.val * asum) / (crnu * nu11);
        r.err = r.err + 8.0 * GSL_DBL_EPSILON * Math.abs(r.val);
    }
...
//    nu = 20: uniformly good to > 13D
//
export function gsl_sf_bessel_Ynu_asymp_Olver_e(nu, x)
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    else
    {
        ...
        var crnu     = nu ** (1.0 / 3.0);
        var nu3      = nu * nu * nu;
        var nu11     = nu3 * nu3 * nu3 * nu * nu;
        var bi       = { val: 0.0, err: 0.0 }; // Result;
        var bip      = { val: 0.0, err: 0.0 }; // Result;
        var asm      = { val: 0.0, err: 0.0 }; // Result;
        ...
        arg  = crnu * crnu * zeta;
        bi  = gsl_sf_airy_Bi_e(arg, GSL_MODE_DEFAULT);
        bip = gsl_sf_airy_Bi_deriv_e(arg, GSL_MODE_DEFAULT);
        
        r.val = -pre * (bi.val * asum / crnu + bip.val * bsum / (nu * crnu * crnu));
        r.err =  pre * (bi.err * Math.abs(asum / crnu));
        r.err = r.err + pre * Math.abs(bi.val) * asum_err / crnu;
        r.err = r.err + pre * Math.abs(bi.val * asum) / (crnu * nu11);
        r.err = r.err + 8.0 * GSL_DBL_EPSILON * Math.abs(r.val);
    }
