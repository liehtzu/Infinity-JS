...
// LogTwoPi : CONSTANT LONG_FLOAT = 1.8378770664093454835606594728111235279723;
...
const zeta_xlt1_cs = { length: 13, c: zeta_xlt1_data, order: 13, a: -1.0, b: 1.0, order_sp: 8 };

// chebyshev fit for (s(t)-1)Zeta[s(t)]
// s(t)= (19t+21)/2
// -1 <= t <= 1
//
const zeta_xgt1_data =
    [
...
const zeta_xgt1_cs = { length: 29, c: zeta_xgt1_data, order: 29, a: -1.0, b: 1.0, order_sp: 17 };

// chebyshev fit for Ln[Zeta[s(t)] - 1 - 2^(-s(t))]
// s(t)= 10 + 5t
// -1 <= t <= 1; 5 <= s <= 15
//
const zetam1_inter_data =
    [
...
const zetam1_inter_cs = { length: 23, c: zetam1_inter_data, order: 22, a: -1.0, b: 1.0, order_sp: 12 };

// ----------------------------------------------------------------------------

// assumes s >= 0 and s != 1.0
//PRAGMA Inline(riemann_zeta_sgt0);
function riemann_zeta_sgt0(s)
{
    var x = 0.0;
    var r = { val: 0.0, err: 0.0 }; // Result;
    var c = { val: 0.0, err: 0.0 }; // R

    if (s < 1.0)
    {
        c = cheb_eval_e(zeta_xlt1_cs, 2.0 * s - 1.0);
        r.val = c.val / (s - 1.0);
        r.err = c.err / Math.abs(s - 1.0) + GSL_DBL_EPSILON * Math.abs(r.val);
    }
    else if (s <= 20.0)
    {
        x = (2.0 * s - 21.0) / 19.0;
        c = cheb_eval_e(zeta_xgt1_cs, x);
        r.val = c.val / (s - 1.0);
        r.err = c.err / (s - 1.0) + GSL_DBL_EPSILON * Math.abs(r.val);
    }
    else
    {
...
//PRAGMA Inline(riemann_zeta1ms_slt0);
function riemann_zeta1ms_slt0(s)
{
    var r = { val: 0.0, err: 0.0 }; // Result;

    if (s > -19.0)
    {
        var x = (-19.0 - 2.0 * s) / 19.0;
        var c = { val: 0.0, err: 0.0 }; // Result;

        c = cheb_eval_e(zeta_xgt1_cs, x);
        r.val = c.val / (-s);
        r.err = c.err / (-s) + GSL_DBL_EPSILON * Math.abs(r.val);
    }
    else
    {
...
// works for 5 < s < 15
function riemann_zeta_minus_1_intermediate_s(s)
{
    var t = 0.0;
    var c = { val: 0.0, err: 0.0 }; // Result;
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    t = (s - 10.0) / 5.0;
    c = cheb_eval_e(zetam1_inter_cs, t);
    r.val = Math.exp(c.val) + 2.0 ** (-s);
    r.err = (c.err + 2.0 * GSL_DBL_EPSILON) * r.val;
    return r;
...
// works well for s > 15
//
function riemann_zeta_minus1_large_s(s)
{
    var a  = Math.pow( 2.0, -s);
    var b  = Math.pow( 3.0, -s);
    var c  = Math.pow( 5.0, -s);
    var d  = Math.pow( 7.0, -s);
    var e  = Math.pow(11.0, -s);
    var f  = Math.pow(13.0, -s);
    var t1 = a + b + c + d + e + f;
    var t2 = a * (b + c + d + e + f) + b * (c + d + e + f) + c * (d + e + f) + d * (e + f) + e * f;
    //
    //t3 : LONG_FLOAT = a*(b*(c+d+e+f) + c*(d+e+f) + d*(e+f) + e*f) +
    //            b*(c*(d+e+f) + d*(e+f) + e*f) +
    //            c*(d*(e+f) + e*f) +
    //            d*e*f;
    //t4 : LONG_FLOAT = a*(b*(c*(d + e + f) + d*(e + f) + e*f) + c*(d*(e+f) + e*f) + d*e*f) +
    //            b*(c*(d*(e+f) + e*f) + d*e*f) +
    //            c*d*e*f;
    //t5 : LONG_FLOAT = b*c*d*e*f + a*c*d*e*f+ a*b*d*e*f+ a*b*c*e*f+ a*b*c*d*f+ a*b*c*d*e;
    //t6 : LONG_FLOAT = a*b*c*d*e*f;
    //
    var numt = t1 - t2; // + t3 - t4 + t5 - t6 */;
    var zeta = 1.0 / ((1.0 - a) * (1.0 - b) * (1.0 - c) * (1.0 - d) * (1.0 - e) * (1.0 - f));

    var r = { val: 0.0, err: 0.0 }; // Result;

    r.val = numt * zeta;
    r.err = (15.0 / s + 1.0) * 6.0 * GSL_DBL_EPSILON * r.val;
    return r;
...
// zeta(n) - 1
const ZETA_POS_TABLE_NMAX = 100;
const zetam1_pos_int_table =
    [
   -1.5,                               // zeta(0)
...
const ZETA_NEG_TABLE_NMAX = 99;
const ZETA_NEG_TABLE_SIZE = 50;
const zeta_neg_int_table =
    [
...
const ETA_POS_TABLE_NMAX = 100;
const eta_pos_int_table =
    [
...
const ETA_NEG_TABLE_NMAX = 99;
const ETA_NEG_TABLE_SIZE = 50;
const eta_neg_int_table =
    [
...
export function gsl_sf_hzeta_e(s, q)
{
    var p1 = 0.0;
    var p2 = 0.0;
    var p3 = 0.0;
    ...
    else
    {
        ...
        else if (s > 0.5 * max_bits && q < 1.0)
        {
            ...
            p3 = (q / (2.0 + q)) ** s;
            r.val = p1 * (1.0 + p2 + p3);
            r.err = GSL_DBL_EPSILON * (0.5 * s + 2.0) * Math.abs(r.val);
        }
        else
        {
            ...
            for (j = 0; j <= jmax; j++)
            {
                delta1 = hzeta_c[j+1] * scp * pcp;
                ans = ans + delta1;
                if (Math.abs(delta1 / ans) < 0.5 * GSL_DBL_EPSILON)
                {
                    break;
                }
                scp = scp * (s + (2 * j + 1)) * (s + (2 * j + 2));
                pcp = pcp / (((kmax) + q) * ((kmax) + q));
            }

            r.val = ans;
            r.err = 2.0 * (jmax + 1) * GSL_DBL_EPSILON * Math.abs(ans);
        }
    }
...
//Math.fmod = function (a,b) { return Number((a - (Math.floor(a / b) * b)).toPrecision(15)); };
...
export function gsl_sf_zeta_e( s )
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    {
        r = riemann_zeta_sgt0( s );
    }
    else
    {
        // reflection formula, [Abramowitz+Stegun, 23.2.5]
        var zeta_one_minus_s = { val: 0.0, err: 0.0 }; // Result;
        var stat_zoms = 0;
        var sin_term = 0.0;

        zeta_one_minus_s = riemann_zeta1ms_slt0( s );
        //IF (fmod(s, 2.0) = 0.0)
        let fm2 = Math.fmod(s, 2.0 );
        let fm4 = Math.fmod(s, 4.0 );
        if ( Math.fmod(s, 2.0 ) == 0.0 )
        {
            sin_term = 0.0;
        }
        else
        {
            //sin_term = Sin(0.5 * M_PI * fmod(s, 4.0)) / M_PI;
            sin_term = Math.sin( 0.5 * M_PI * Math.fmod( s, 4.0 ) ) / M_PI;
        }
        if ( sin_term == 0.0 )
        {
            r.val = 0.0;
        ...
        else if ( s > -170.0 )
        {
            ...
            n  = Math.trunc(Math.floor((-s) / 10.0));
            fs = s + 10.0 * (n);
            p  = Math.pow(2.0 * M_PI, fs) / twopi_pow[n];
            g = gsl_sf_gamma_e(1.0 - s);
            r.val = p * g.val * sin_term * zeta_one_minus_s.val;
            r.err = Math.abs(p * g.val * sin_term) * zeta_one_minus_s.err;
            r.err = r.err + Math.abs(p * sin_term * zeta_one_minus_s.val) * g.err;
            r.err = r.err + GSL_DBL_EPSILON * (Math.abs(s) + 2.0) * Math.abs(r.val);
        }
        else
        {
...
export function gsl_sf_zeta_int_e(n)
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    if (n < 0)
    {
        if (! GSL_IS_ODD(n))
        {
            r.val = 0.0; // exactly zero at even negative integers
            r.err = 0.0;
        }
        else if (n > -ZETA_NEG_TABLE_NMAX)
        {
            r.val = zeta_neg_int_table[-(n+1)/2];
            r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
        }
        else
        {
    ...
    {
        r.val = 1.0 + zetam1_pos_int_table[n];
        r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
    }
    else
    {
...
export function gsl_sf_zetam1_e( s )
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    {
        return riemann_zeta_minus_1_intermediate_s( s );
    }
    else
    {
        return riemann_zeta_minus1_large_s( s );
    }
...
export function gsl_sf_zetam1_int_e(n)
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    if (n < 0)
    {
        if (! GSL_IS_ODD(n))
        {
            r.val = -1.0; // at even negative integers zetam1 = -1 since zeta is exactly zero
            r.err = 0.0;
            return r;
        }
        else if (n > -ZETA_NEG_TABLE_NMAX)
        {
            r.val = zeta_neg_int_table[-(n+1)/2] - 1.0;
            r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
            return r;
        }
        else
    ...
    {
        r.val = zetam1_pos_int_table[n];
        r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
        return r;
    }
    else
    {
        return gsl_sf_zetam1_e((n));
    }
...
export function gsl_sf_eta_int_e(n)
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    else
    {
        ...
        {
            r.val = eta_neg_int_table[-(n + 1) / 2];
            r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
        }
        else
        {
            var z = { val: 0.0, err: 0.0 }; // Result;
            var p = { val: 0.0, err: 0.0 }; // Result;

            z = gsl_sf_zeta_int_e(n);
            p = gsl_sf_exp_e((1.0 - (n)) * M_LN2);
            r = gsl_sf_multiply_e(-p.val, z.val);
            r.err = Math.abs(p.err * (M_LN2 * (1.0 - (n))) * z.val) + z.err * Math.abs(p.val);
...
export function gsl_sf_eta_e(s)
{
    var r = { val: 0.0, err: 0.0 }; // Result;
    ...
    else if (Math.abs(s - 1.0) < 10.0 * GSL_ROOT5_DBL_EPSILON)
    {
        const del = s - 1.0;
        const c0  = M_LN2;
        const c1  = M_LN2 * (M_EULER - 0.5 * M_LN2);
        const c2  = -0.0326862962794492996;
        const c3  =  0.0015689917054155150;
        const c4  =  0.00074987242112047532;

        r.val = c0 + del * (c1 + del * (c2 + del * (c3 + del * c4)));
        r.err = 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
        return r;
    }
    else
    {
        ...
        z = gsl_sf_zeta_e(s);
        p = gsl_sf_exp_e((1.0 - s) * M_LN2);
        r = gsl_sf_multiply_e(1.0 - p.val, z.val);
        r.err = Math.abs(p.err * (M_LN2 * (1.0 - s)) * z.val) + z.err * Math.abs(p.val);
        r.err = r.err + 2.0 * GSL_DBL_EPSILON * Math.abs(r.val);
        return r;
    }
...
export function gsl_sf_zeta( s )
{ // gsl_sf_zeta
    return EVAL_RESULT_D( gsl_sf_zeta_e, s, "gsl_sf_zeta" );
} // gsl_sf_zeta

export function gsl_sf_hzeta( s, a )
{ // gsl_sf_hzeta
    return EVAL_RESULT_DD( gsl_sf_hzeta_e, { x: s, y: a }, "gsl_sf_hzeta" );
} // gsl_sf_hzeta

export function gsl_sf_zeta_int (s )
{ // gsl_sf_zeta_int
    return EVAL_RESULT_I( gsl_sf_zeta_int_e, s, "gsl_sf_zeta_int" );
} // gsl_sf_zeta_int

export function gsl_sf_zetam1( s )
{ // gsl_sf_zetam1
    return EVAL_RESULT_D( gsl_sf_zetam1_e, s, "gsl_sf_zetam1" );
} // gsl_sf_zetam1

export function gsl_sf_zetam1_int( s )
{ // gsl_sf_zetam1_int
    return EVAL_RESULT_I( gsl_sf_zetam1_int_e, s, "gsl_sf_zetam1_int" );
} // gsl_sf_zetam1_int

export function gsl_sf_eta_int( s )
{ // gsl_sf_eta_int
    return EVAL_RESULT_I( gsl_sf_eta_int_e, s, "gsl_sf_eta_int" );
} // gsl_sf_eta_int

export function gsl_sf_eta( s )
{ // gsl_sf_eta
    return EVAL_RESULT_D( gsl_sf_eta_e, s, "gsl_sf_eta" );
} // gsl_sf_eta
